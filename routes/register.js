...
passwordless.init(new MemoryTokenStore(), { userProperty: 'email' });
...
module.exports = function (express, secret, User) {			
	...
	router.post('/register',
		...
		passwordless.requestToken(function(user, delivery, callback, req) {
			...
			User.findOne({ name: name, email: email }, function(err, usr) {
	...
	router.post('/login', passwordless.acceptToken({ uidField: 'email', allowPost: true }),
		...
		function(req, res) {
			User.findOne({ email: req.email }, function(err, user) {
				...
				var token = Jwt.sign({
					...
					uid: user._id,
					device: device
				}, secret, { expiresInMinutes: 300000});
				user.devices.push({ name: device, jti: jti });
